...
1. Run `detect_command` — stdout must exactly match one slot `path`, or be empty (first deploy → `slots[0]`).
...
These are injected into command execution only — they do not appear in `env_file` or `{{VAR}}` substitution.
