...
3. Run `pre_commands` (default cwd = idle slot / `target`; use `cwd: project` for manifest-dir scripts; slot env vars available).
...
8. Run `post_commands` (same cwd rules as pre).
